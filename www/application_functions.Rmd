...
---
### Function to make predictions using the preloaded trained model and test data
```{r}
...
### Function to plot model characteristics of true positives, true negatives, false positives, and false negatives
...
### Function to display model accuracy metrics as a barplot
...
### Function that plots the model predictions in a PC vector space 
...
pca_accuracy_plotter<-function(test_data,model,space='3D'){
  ...
  pca_result<<-prcomp(test_data_dummy, 
                     scale.=T)
  ...
  if (space =='3D'){
    plot<-plot_ly(pca_df, x = ~PC1, y = ~PC2, z = ~PC3, color = ~accuracy,
      ...
      layout(
        ...
        legend = list(font = list(color = colors$body_color)), 
        scene = list(
          xaxis = list(
            title='PC1',
...
### Function to return the most dominant variables in a principal component 
...
### Function that plots a stacked barchart for each categorical variable
...
plot_categorical_variables <- function(df) {
  ...
  # Remove the 'Country' variable > used for atual mapping
  ...
  plots <- list()  
  num_plots <- sum(categorical_cols)
  ...
  for(var in names(categorical_cols)) {
    ...
    if (var == 'Alzheimer.s.Diagnosis'){
      text_color<-colors$danger #highlight the outcome variable 
    }else{
    ...
    #use subplot to arrange in 4x5 grid
    subplot(plots,
            nrows=4,
            titleX=T,
...
### Function to plot numerical variables as histograms
...
### Function to create receiver operating characteristics (ROC) curves
...
### Function to return the decision tree corresponding to an rpart model
